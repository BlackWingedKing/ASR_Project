...
def download_video(start_num, yt_id, start_time, end_time, fps, aud_sampling, path_vid, path_snippet, path_shift,
                            path_unshift, rng, njobs):
    link = ["https://www.youtube.com/watch?v="+id for id in yt_id]
    for i in range(len(link)):
        download_yt_link(start_num+i+1, link[i], start_time[i], end_time[i], fps,
                         aud_sampling, path_vid, path_snippet, 
                         path_shift, path_unshift, rng.rand())
...
def download_video_parallel(start_num, yt_id, start_time, end_time, fps, aud_sampling, path_vid, path_snippet, path_shift,
                            path_unshift, rng, njobs):
    link = ["https://www.youtube.com/watch?v="+id for id in yt_id]
    Parallel(n_jobs=njobs, prefer="threads")(delayed(download_yt_link)(
        start_num+i+1, link[i], start_time[i], end_time[i], fps,
...
#     start_from = len(os.listdir('data/train/full_vid/'))+1
    ...
    download_video(start_num, audioset.iloc[start_num:start_num+num_videos, 0], audioset.iloc[start_num:start_num+num_videos, 1],
